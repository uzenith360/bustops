...
function Place(info, options, onClick) {
    this._infowindow;
    this._marker;
    ...
    var googleMaps = google.maps, infowindow = new googleMaps.InfoWindow({
        content: '<div style="display: inline;">' + (info.icon ? '<img src="' + info.icon + '" width="16" height="16">' : '') + (names ? '<strong>' + names + '</strong>' : '') + (addresses ? '<br/><span>' + addresses + '</span>' : '') + (info.description ? '<hr style="margin:8px 0 8px 0;"><span style="color: #999;">' + info.description + '</span>' : '') + '</div>'
